...
func (b *Builder) BuildTemplate(lxcfile *models.LXCfile, templateName string, buildArgs map[string]string) (*BuildResult, error) {
	...
	if err := b.createTempContainer(containerID, lxcfile.From); err != nil {
		return nil, fmt.Errorf("failed to create temporary container: %w", err)
	}
	
	// Ensure cleanup happens even if build fails
	defer func() {
		if err := b.cleanupTempContainer(containerID); err != nil {
			b.logError("Failed to cleanup temporary container %d: %v", containerID, err)
		}
	}()
	
	// Start the container for configuration
	if err := b.startContainer(containerID); err != nil {
		return nil, fmt.Errorf("failed to start temporary container: %w", err)
	}
	
	// Wait for container to be ready
	if err := b.waitForContainer(containerID); err != nil {
